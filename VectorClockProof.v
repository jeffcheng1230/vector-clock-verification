	...
	- intros comp Hlen a p q Ha_bound Ha_proc.
		...
		}
		{
			destruct Hcomp' as [tl Hcomp]. {
				...
				rewrite Ha_proc. rewrite Hcomp. rewrite (comp_step comp' tl (length comp)). unfold step.
				...
				unfold event_process. rewrite Ha. rewrite (nth_middle comp' nil tl (Event 0)).
					destruct tl; simpl; simpl_step; rewrite (n_value comp'); rewrite Nat.eqb_refl; rewrite Nat.eqb_refl; destruct (proc =? p) eqn:Hproc_p; lia.
				}
				{
					(* use IH *)
					pose proof (comp'_len comp comp' tl Hcomp) as Hcomp'.
					assert (a < length comp') as Ha_bound'. { lia. }
					rewrite Ha_proc. rewrite Hcomp. rewrite <- (preserve_ets comp' tl a Ha_bound').

					rewrite (comp_step comp' tl (length comp)). unfold step. apply Nat.eqb_neq in Ha_neq.
					...
					destruct tl. {
						...
						destruct Hproc_p as [Hproc_p | Hproc_p_neq]. {
							destruct (proc =? q) eqn:Hp_ap. {
								rewrite Hcomp in Ha_proc. rewrite <- Ha_proc. rewrite Hp_ap.
								rewrite Hproc_p. rewrite Nat.eqb_refl.
								apply (Nat.le_trans
									(snd (run_computation comp' init_state) a p)
									(snd (fst (run_computation comp' init_state)) q p)
									(snd (fst (run_computation comp' init_state)) p p + 1)).
								{ apply IHn_hyp. }
								{ apply Nat.eqb_eq in Hp_ap. rewrite <- Hp_ap. rewrite Hproc_p. lia. }
							}
							{
								rewrite Hcomp in Ha_proc. rewrite <- Ha_proc. rewrite Hp_ap. apply IHn_hyp.
							}
						}
						{
							apply Nat.eqb_neq in Hproc_p_neq. rewrite Hcomp in Ha_proc. rewrite <- Ha_proc.
							destruct (proc =? q) eqn:Hproc_q. {
								rewrite Hproc_p_neq. apply Nat.eqb_eq in Hproc_q. rewrite Hproc_q. apply IHn_hyp.
							}
							{
								apply IHn_hyp.
							}
					...
					}
					{
						...
						destruct Hproc_p as [Hproc_p | Hproc_p_neq]. {
							destruct (proc =? q) eqn:Hp_ap. {
								rewrite Hcomp in Ha_proc. rewrite <- Ha_proc. rewrite Hp_ap.
								rewrite Hproc_p. rewrite Nat.eqb_refl. rewrite orb_true_l.

								destruct (
									snd (run_computation comp' init_state) send_event_num p <? snd (fst (run_computation comp' init_state)) p p
								) eqn:Hmax. {
									apply Nat.eqb_eq in Hp_ap. rewrite <- Hp_ap in IHn_hyp. rewrite Hproc_p in IHn_hyp.
									apply (Nat.le_trans
										(snd (run_computation comp' init_state) a p)
								...
								{
									apply Nat.ltb_nlt in Hmax. apply Nat.nlt_ge in Hmax. apply Nat.eqb_eq in Hp_ap. rewrite <- Hp_ap in IHn_hyp. rewrite Hproc_p in IHn_hyp.
									apply (Nat.le_trans
										(snd (run_computation comp' init_state) a p)
						...
						{
							rewrite Hcomp in Ha_proc. rewrite <- Ha_proc.
							destruct (proc =? q) eqn:Hproc_q. {
								destruct (snd (run_computation comp' init_state) send_event_num p <? snd (fst (run_computation comp' init_state)) proc p) eqn:Hmax. {
									apply Nat.eqb_neq in Hproc_p_neq. rewrite Hproc_p_neq. apply Nat.eqb_eq in Hproc_q. rewrite Hproc_q. apply IHn_hyp.
								}
								{
									apply Nat.eqb_eq in Hproc_q. apply Nat.eqb_neq in Hproc_p_neq. rewrite Hproc_p_neq.
									apply Nat.ltb_nlt in Hmax. apply Nat.nlt_ge in Hmax.
									destruct (send_event_proc =? p). {
										simpl. rewrite Hproc_q in Hmax.
										apply (Nat.le_trans
											(snd (run_computation comp' init_state) a p)
											(snd (run_computation comp' init_state) send_event_num p)
											(snd (run_computation comp' init_state) send_event_num p + 1)).
...
(*Lemma sequential:
	forall (comp : computation) (a b n : nat) (p : process) (e f : event),
	n = length comp ->
	...
	n >= 2 ->
	p = event_process comp a ->
	event_process comp a = event_process comp b ->
	let event_timestamps : time_stamps :=
		snd (run_computation comp init_state) in
	event_timestamps a p < event_timestamps b p ->
	seq comp a b.
Proof.
	intros comp a b n p e f H0 H1 H2 H3 H4 H5 H6 H7 H8.
	unfold seq.
	split.
	- apply H6.
	- destruct n.
		...
		+ destruct n. {
			...
			}
			{
				...
				}
				{
					induction n. {
						destruct Hcomp' as [tl Hcomp]. {
							destruct (b =? length comp') eqn:Hb. {
								admit.
							}
							{
								destruct (a =? length comp') eqn:Ha. {
									(* contradict H8 *)
									assert (b < length comp') as Hb'. {
										(* b < length comp' *)
										admit.
									}
									unfold H7 in H8.
									assert (
										...
										rewrite (comp_tail comp' tl). {
											destruct tl. {
												simpl. unfold update_clock. unfold update_event_time. unfold update.
												apply beq_nat_true in Ha. rewrite Hcomp in H5.
												unfold event_process in H5. rewrite Ha in H5.
												rewrite (nth_middle comp' nil (Event proc) (Event 0)) in H5.
												rewrite H5. rewrite Nat.eqb_refl.
												rewrite (n_value comp').
												{
													rewrite <- Ha.
													unfold event_eqb. rewrite Nat.eqb_refl. rewrite Nat.eqb_refl.
													rewrite plus_comm. unfold plus. apply Nat.lt_succ_diag_r.
												}
											}
											{
												admit.
									...
									assert (
										...
										rewrite Hcomp. rewrite <- (preserve_ets comp' tl b (length comp') p). {
											apply (final_clocks comp' b (length comp') p). {
												admit.
											}
											{
												apply Hb'.
									...
									assert (
										...
										Search lt le.
										apply Nat.le_lt_trans with (m:= snd (fst (run_computation comp' init_state)) p p). {
											apply Htrans1.
										}
										{
											apply Htrans2.
									...
									apply (Arith_prebase.lt_not_le_stt (snd (run_computation comp init_state) b p) (snd (run_computation comp init_state) a p)) in contra.
									unfold not in contra.
									apply (Nat.lt_le_incl (snd (run_computation comp init_state) a p) (snd (run_computation comp init_state) b p)) in H8.
									apply contra in H8. inversion H8.
								}
								{
									admit.
								}
...
(*Lemma sp_non_inc:
	forall (comp : computation) (a c : nat) (p : process),

	Admitted.*)

(*Lemma non_reflexive:
	forall (comp : computation) (a b : nat),
	after a b \/ after b a*)
...
(*Inductive after : computation -> nat -> nat -> Prop :=
	| after_sp (comp : computation) (a b : nat) (H : seq comp a b) : after comp a b
	| after_dp (comp : computation) (a b : nat) (H : msg comp a b) : after comp a b
	| after_trans (comp : computation) (a b c : nat) (H1 : after comp a b) (H2 : after comp b c) : after comp a c.*)
...
(*Lemma ets_fc:
	forall (comp : computation) (a : nat), (* needs to be tail? *)
	let clocks_final : clocks :=
		snd (fst (run_computation comp init_state)) in
	let event_timestamps : time_stamps :=
		snd (run_computation comp init_state) in
	event_timestamps a = clocks_final (event_process comp a).
Proof.
	intros. unfold event_timestamps. unfold clocks_final.
	destruct (list_tl comp) as [comp' Hcomp']. {
		admit. (* reference old proof of this *)
	}
	{
		destruct Hcomp' as [tl Hcomp]. {
			rewrite Hcomp. rewrite (comp_step comp' tl). {
...
Lemma op_step:
	forall (comp : computation) (tl : event) (state : run_op_state) (a : operation_index),
	let op_state1 := apply_operations (comp ++ tl :: nil) state a in
	let op_state2 := (apply_op a) (apply_operations comp state a) tl in
	op_state1 = op_state2.
Proof.
	intros. unfold op_state1. unfold op_state2.
	apply (fold_left_app (apply_op a) comp (tl :: nil) state0).
Qed.

Lemma n_rem_op:
	forall (comp : computation) (db_state : run_op_state) (a : operation_index),
	fst (fst (apply_operations comp db_state a)) = length comp.
Proof.
	Admitted.

Lemma preserve_op:
	forall (comp : computation) (db_state : run_op_state),
	apply_operations comp db_state (length comp + 1) =
	apply_operations comp db_state (length comp).
Proof.
	Admitted.

Lemma ets_fc3:
	forall (comp : computation) (p : process) (a : operation_index) (db_state : run_op_state),
	(exists (x : nat), x < length comp /\ event_process comp x = p) ->
	let clocks_final : clocks :=
		snd (fst (run_computation comp init_state)) in
	let event_timestamps : time_stamps :=
		snd (run_computation comp init_state) in
	let db_states_rem_a :=
		snd (fst (apply_operations comp db_state a)) in
	let op_states_rem_a :=
		snd (apply_operations comp db_state a) in
	let db_states_rem_len :=
		snd (fst (apply_operations comp db_state (length comp))) in
	let op_states_rem_len :=
		snd (apply_operations comp db_state (length comp)) in
	exists (y : nat), y < length comp /\ event_process comp y = p /\ event_timestamps y = clocks_final p /\
	op_states_rem_a y = db_states_rem_a p /\ op_states_rem_len y = db_states_rem_len p.
Proof.
	intros comp. remember (length comp) as n. generalize dependent comp.
	induction n. {
		intros. destruct H as [x [H1 H2]]. inversion H1.
	...
	{
		(*intros comp H H0 Hx_exists. *)
		intros comp Hlen p a db_state Hx_exists.
		destruct (list_tl comp) as [comp' Hcomp']. {
			symmetry in Hlen. rewrite Hlen. lia.
		}
		{
			destruct Hcomp' as [tl Hcomp]. {
				...
				assert ((fst (fst (run_computation comp' init_state)) =? length comp - 1) = true) as Hcomp'_len1. {
					rewrite (n_value comp'). rewrite Hcomp'. rewrite Nat.eqb_refl. reflexivity.
				}
				assert ((fst (fst (apply_operations comp' db_state a)) =? length comp - 1) = true) as Hcomp'_len2. {
					rewrite (n_rem_op comp'). rewrite Hcomp'. rewrite Nat.eqb_refl. reflexivity.
				}
				assert ((fst (fst (apply_operations comp' db_state a)) =? a) = false) as Hcomp'_len3. {
					admit.
				}
				assert (((fst (fst (apply_operations comp' db_state (length comp)))) =? length comp - 1) = true) as Hcomp'_len4. {
					admit.
				}
				assert (((fst (fst (apply_operations comp' db_state (length comp)))) =? length comp) = false) as Hcomp'_len5. {
					admit.
				}
				(* ==== cccccccccc ====== - afterwards, go back to fix a' < a case of trans_contra *)
				destruct Htl as [Htl_p | Htl_notp]. {
					...
					rewrite Hlen.
					rewrite (comp_step comp' tl).
					rewrite (op_step comp' tl db_state a).
					rewrite (op_step comp' tl db_state (length comp)).
					unfold step. unfold apply_op.
					destruct tl. {
						simpl_step. rewrite <- Hcomp.
						rewrite Hcomp'_len1. rewrite Hcomp'_len3. rewrite Hcomp'_len5.
						rewrite Nat.eqb_refl. simpl. rewrite Hcomp'_len2. rewrite Hcomp'_len4.

						simpl. unfold event_process in Htl_p. rewrite <- Hcomp' in Htl_p. rewrite Hcomp in Htl_p.
						rewrite (nth_middle comp' nil (Event proc) (Event 0)) in Htl_p.
						rewrite Htl_p. rewrite Nat.eqb_refl. split. reflexivity. split. reflexivity. reflexivity.
					}
					{
						simpl_step. rewrite <- Hcomp.
						rewrite Hcomp'_len1. rewrite Hcomp'_len3. rewrite Hcomp'_len5.
				...
				}
				{
					...
					destruct Hx as [Hold_x | Hno_x]. {
						rewrite <- Hlen in Hcomp'. simpl in Hcomp'. rewrite Nat.sub_0_r in Hcomp'. symmetry in Hcomp'. simpl. simpl in IHn.
						rewrite <- Hcomp' in Hold_x.
						apply (IHn comp' Hcomp' p a db_state) in Hold_x as IHn_app. {
							simpl. destruct IHn_app as [y IHn_app]. exists y.
							split. {
								destruct IHn_app as [Hy_bound [Hep_y IHn_app]]. apply Nat.lt_lt_succ_r in Hy_bound. apply Hy_bound.
							}
							...
							{
								rewrite Hcomp. rewrite Hlen.
								rewrite <- (preserve_ets comp' tl).
								rewrite (comp_step comp' tl).
								rewrite (op_step comp' tl db_state a).
								rewrite (op_step comp' tl db_state (length comp)).
								destruct IHn_app as [Hy_bound [Hep_y IHn_app]].
								...
								rewrite (nth_middle comp' nil tl (Event 0)) in Htl_notp. apply Nat.eqb_neq in Htl_notp.
								unfold step. unfold apply_op.
								rewrite Hcomp'_len3. simpl.
								assert ((fst (fst (apply_operations comp' db_state a)) =? y) = false) as Hcomp'_len6. {
									admit. (* can lia after some manip *)
								}
								assert ((fst (fst (apply_operations comp' db_state (length comp))) =? y) = false) as Hcomp'_len7. {
									admit. (* can lia after some manip *)
								}
								assert (length comp = length comp' + 1) as Hcomp'_len8. { lia. }
								destruct tl; simpl_step; rewrite Htl_notp;
								rewrite Hcomp'_len6; rewrite Hcomp'_len5; simpl;
								rewrite Hcomp'_len7; rewrite Htl_notp;
								rewrite Hcomp'_len8; rewrite (preserve_op comp' db_state); rewrite <- Hcomp';
								apply IHn_app.
								lia.
							}
						}
...
Lemma ets_aop_eq_aux:
	forall (comp : database_computation) (a b : operation_index) (p q : user) (db_state : database_states),
	a < length comp ->
	b < length comp ->
	p = event_process comp a ->
	q = event_process comp b ->

	let event_timestamps : time_stamps := 
		snd (run_computation comp init_state) in
	event_timestamps a p < event_timestamps b p
	<->
	let op_states := snd (apply_operations comp (0, db_state, fun _ => 0) (length comp)) in
	let op_states_rem := snd (apply_operations comp (0, db_state, fun _ => 0) a) in
	op_states b <> op_states_rem b.
Proof.
	intros comp. remember (length comp) as n. generalize dependent comp.
	destruct n.
	- admit.
	- destruct n.
		...
		{
			simpl. intros comp Hlen a b p q db_state Ha_bound Hb_bound Ha_proc Hb_proc. remember (0, db_state, fun _ : operation_index => 0) as init_op_state.
			destruct (list_tl comp) as [comp' Hcomp']. {
				symmetry in Hlen. rewrite Hlen. lia.
			}
			{
			destruct Hcomp' as [tl Hcomp]. {
				assert (a < length comp') as Ha_bound''. { admit. }
				pose proof (comp'_len comp comp' tl Hcomp) as Hcomp'. rewrite <- Hlen in Hcomp'. simpl in Hcomp'. symmetry in Hcomp'.

				assert (exists x : nat, x < length comp' /\ event_process comp' x = q) as Hx. {
					admit.
					(*destruct Hx as [x [Hx_bound Hx_proc]]. exists x.
					split. lia. rewrite Hcomp. rewrite (preserve_proc comp' tl x) in Hx_proc. apply Hx_proc. lia.*)
				}
				rewrite Hlen.
				apply (ets_fc3 comp' q (length comp') init_op_state) in Hx as Hx_rem.
				apply (ets_fc3 comp' q a init_op_state) in Hx. (* probably have to do this IH twice, one for each y for Hx', Hx'_rem *)

				destruct Hx_rem as [x [Hx_bound [Hx_proc [Hx_ets_fc Hx_aop_db]]]].
				destruct Hx as [y [Hy_bound [Hy_proc [Hy_ets_fc Hy_aop_db]]]].

				assert (
					snd (run_computation comp' init_state) a p < snd (run_computation comp' init_state) y p <->
					snd (apply_operations comp' init_op_state (length comp')) y <> snd (apply_operations comp' init_op_state a) y
				) as Heq'. {
					destruct (list_tl comp') as [comp'' Hcomp'']. { lia. }
					{
						destruct Hcomp'' as [tl' Hcomp'']. {
							rewrite <- Hcomp' in Ha_bound''. rewrite <- Hcomp' in Hy_bound. symmetry in Hy_proc.
							pose proof (IHn comp' Hcomp' a y p q db_state Ha_bound'' Hy_bound).
							simpl in H. rewrite Heqinit_op_state. rewrite <- Hcomp'. apply H.
							{ rewrite Hcomp' in Ha_bound''. rewrite Hcomp in Ha_proc. rewrite (preserve_proc comp' tl a). apply Ha_proc. apply Ha_bound''. }
							{ apply Hy_proc. }
						}
					}
				}

				assert (b = length comp') as Hb. { admit. }

				unfold event_process in Hb_proc. rewrite Hcomp in Hb_proc. rewrite Hb in Hb_proc.
				rewrite (nth_middle comp' nil tl (Event 0)) in Hb_proc.

				remember (snd (run_computation comp init_state) a p) as rc_a.
				rewrite Hcomp. rewrite (comp_step comp' tl).
				rewrite (op_step comp' tl init_op_state (length (comp' ++ tl :: nil))). rewrite (op_step comp' tl init_op_state a). 
				unfold step. unfold apply_op. simpl_step. 
				destruct tl. {
					simpl. rewrite (n_value comp'). rewrite Hb. rewrite Nat.eqb_refl. rewrite Nat.eqb_refl.
					rewrite (n_rem_op comp' init_op_state (length (comp' ++ Event proc :: nil))).
					rewrite (n_rem_op comp' init_op_state a).
					assert ((length comp' =? length (comp' ++ Event proc :: nil)) = false) as Hcomp_neq. { admit. }
					assert ((length comp' =? a) = false) as Ha_neq. { admit. }
					rewrite Hcomp_neq. rewrite Ha_neq. simpl. rewrite Nat.eqb_refl. rewrite <- Hb_proc.
					assert (
						forall (a b : nat),
						a + 1 <> b + 1 <->
						a <> b) as Hneq_S. { lia. }
					...
					split. {
						rewrite Heqrc_a. intros Hets. apply Hneq_S.
						destruct (q =? p) eqn:Hqp. {
							apply Nat.eqb_eq in Hqp. rewrite <- Hqp in Hets.
							rewrite <- Hy_ets_fc in Hets.
							assert (
								forall (tl : event),
								snd (fst (apply_operations comp' init_op_state (length (comp' ++ tl :: nil)))) =
								snd (fst (apply_operations comp' init_op_state (length comp')))
							) as Hpreserve_aop. { admit. }
							rewrite Hpreserve_aop.
							rewrite <- Hx_aop_db.
							rewrite <- Hy_aop_db.
							(* assert that seq comp a y -> after comp a y -> ets a p < ets y p, don't need assumption *)
						}
						apply Hneq_S in Hets.
					}

					Check preserve_ets.
					rewrite (preserve_ets comp' (Event proc) y).

				}
				
				assert (snd (run_computation comp init_state) b p = 0) as Hb_step. {
					rewrite Hcomp. rewrite (comp_step comp' tl). unfold step. simpl_step.
				}


				assert (
					snd (run_computation comp' init_state) y p < 
					snd (run_computation comp init_state) b p
				) as Hyb. {
					assert (after comp y b) as Hafter_yb. {
						apply (after_sp comp y b). unfold seq.
						split. { rewrite (preserve_proc comp' tl y) in Hy_proc. rewrite Hcomp in Hb_proc. rewrite <- Hy_proc in Hb_proc. rewrite Hcomp. apply Hb_proc. apply Hy_bound. }
						{ assert (b = length comp') as Hb. { admit. } lia. }
					}
					(*apply order in Hafter_yb.*)
					admit.
				}
				...
				rewrite (preserve_ets comp' tl a Ha_bound'') in Heq'.


				assert (
					forall (a : operation_index),
					exists (y : nat),
					y < length comp /\
					event_process comp y = p /\
					snd (run_computation comp init_state) y = snd (fst (run_computation comp init_state)) p /\
					snd (apply_operations comp init_op_state a) y = snd (fst (apply_operations comp init_op_state a)) p
				) as Hets_fc. {
					assert (exists x : nat, x < length comp /\ event_process comp x = q) as Hx'. {
						destruct Hx as [x [Hx_bound Hx_proc]]. exists x.
						split. lia. rewrite Hcomp. rewrite (preserve_proc comp' tl x) in Hx_proc. apply Hx_proc. lia.
					...
					intros.
					pose proof (ets_fc3 comp q a0 init_op_state) as Hets_fc3. simpl in Hets_fc3.
					apply Hets_fc3 in Hx'.
				}
				Check ets_fc3.
				pose proof ets_fc3 as Hets_fc3. simpl in Hets_fc3.
		}
...
Definition keep {A} (ind : nat) (acc: nat * list A) (e : A) : nat * list A :=
	let n := fst acc in
	let l := snd acc in
	if n =? ind then (n-1, l)
	else (n-1, l ++ e :: nil).

Definition rem_op_aux {A} (l : list A) (ind : nat) : nat * list A :=
	fold_left (keep ind) l (0, nil).

Definition rem_op {A} (l : list A) (ind : nat) : list A :=
	snd (rem_op_aux l ind).

Lemma op_step:
	forall (comp : computation) (tl : event) (state : run_op_state),
	let op_state1 := apply_operations (comp ++ tl :: nil) state in
	let op_state2 := apply_op (apply_operations comp state) tl in
	op_state1 = op_state2.
Proof.
	intros. unfold op_state1. unfold op_state2.
	apply (fold_left_app apply_op comp (tl :: nil) state0).
Qed.

Lemma rem_op_tl:
	forall (comp : computation) (tl : event) (a : nat),
	a = length comp ->
	rem_op (comp ++ tl :: nil) a = comp.
Proof.
	Admitted.

Lemma rem_op_comp:
	forall (comp : computation) (tl : event) (state : run_op_state) (a : nat),
	a < length comp ->
	rem_op (comp ++ tl :: nil) a = (rem_op comp a) ++ tl :: nil.
Proof.
	Admitted.

Lemma time_rem:
	forall (comp : database_computation) (a : operation_index) (op_tl : database_operation) (p q : user) (db_state : database_states),
	p = event_process comp a ->
	q = event_process (comp ++ op_tl :: nil) (length comp) ->
	after (comp ++ op_tl :: nil) a (length comp) <->
	let db_states := snd (fst (apply_operations comp (0, db_state, fun _ => 0))) in
	let db_states_rem := snd (fst (apply_operations (rem_op comp a) (0, db_state, fun _ => 0))) in
	db_states q > db_states_rem q.
Proof.
	intros comp. remember (length comp) as n. generalize dependent comp.
	destruct n.
	- admit.
	- destruct n.
		...
		{
			intros comp Hlen a op_tl p q db_state Ha_proc Hb_proc. remember (0, db_state, fun _ : operation_index => 0) as init_op_state. simpl. split. {
				intros Hafter.
				destruct (list_tl comp) as [comp' Hcomp']. {
					symmetry in Hlen. rewrite Hlen. lia.
				}
				{
					destruct Hcomp' as [tl Hcomp]. {
						assert (a < length comp') as Ha_bound''.
						assert (
							snd (fst (apply_operations (comp' ++ op_tl :: nil) init_op_state)) q >
							snd (fst (apply_operations (rem_op (comp' ++ op_tl :: nil) a) init_op_state)) q
						) as Hcmp_base. {
							assert (after (comp' ++ op_tl :: nil) a (length comp')). 
						}

						rewrite Hcomp.
						assert (a < length comp') as Ha_bound'. { admit. }
						rewrite (rem_op_comp comp' tl init_op_state a Ha_bound').
						rewrite (op_step comp' tl init_op_state).
						rewrite (op_step (rem_op comp' a) tl init_op_state).
						unfold apply_op.
						destruct tl. {
							simpl_step.
							destruct (proc =? q). {
	...
	let event_timestamps : time_stamps := 
		snd (run_computation comp init_state) in
	event_timestamps a u < event_timestamps b u <->
	let op_states := snd (apply_operations comp (0, db_state, fun _ => 0)) in
	let op_states_rem := snd (apply_operations (rem_op comp a) (0, db_state, fun _ => 0)) in
	op_states b <> op_states_rem b.

Lemma timestamp_eq:
	forall (comp : database_computation) (a b : operation_index) (u : user) (db_state : database_states),
	u = event_process comp a ->
	let event_timestamps : time_stamps := 
		snd (run_computation comp init_state) in
	event_timestamps a u < event_timestamps b u <->
	let op_states := snd (apply_operations comp (0, db_state, fun _ => 0)) in
...
Proof.
	...
	- destruct n.
		...
		{
			intros comp Hlen a b u db_state Ha_proc. remember (0, db_state, fun _ : operation_index => 0) as init_op_state. simpl. split. {
				intros Hets. 
				destruct (list_tl comp) as [comp' Hcomp']. {
					symmetry in Hlen. rewrite Hlen. lia.
				}
				{
					destruct Hcomp' as [tl Hcomp]. {
						assert (b = length comp' \/ ~(b = length comp')) as Hb. { apply classic. }
						assert (a < length comp') as Ha_bound'. { admit. }
						destruct Hb as [Hb_tl | Hb_ntl]. {
							rewrite Hcomp.
							rewrite (rem_op_comp comp' tl init_op_state a Ha_bound').
							rewrite (op_step comp' tl init_op_state).
							rewrite (op_step (rem_op comp' a) tl init_op_state).
							unfold apply_op.
							rewrite Hcomp in Hets. rewrite (comp_step comp' tl) in Hets. unfold step in Hets.
							destruct tl. {
								simpl in Hets. unfold update_event_time in Hets. unfold update_clock_receive in Hets. unfold update_clock in Hets. unfold update in Hets. unfold event_eqb in Hets.
								simpl_step.
								assert ((fst (fst (run_computation comp' init_state))) = (fst (fst (apply_operations comp' init_op_state)))) as Hfst. { admit. }
								rewrite <- Hfst. (* needs to update (rem_op comp' a) same as comp' *)
								rewrite (n_value comp'). rewrite Hb_tl. rewrite Nat.eqb_refl.
								rewrite (n_value comp') in Hets. rewrite Hb_tl in Hets. rewrite Nat.eqb_refl in Hets.
								assert ((fst (fst (apply_operations (rem_op comp' a) init_op_state)) =? length comp') = false) as Hfst_rem. { admit. }
								rewrite Hfst_rem.

								assert ((length comp' =? a) = false) as Ha_bound''. { admit. }
								rewrite Ha_bound'' in Hets. rewrite Nat.eqb_refl in Hets.
							}
						}
				}
		}
	...
	intros comp a b u db_state Ha_proc. simpl. split.
	- 



(*Lemma rem_step:
	forall (comp : computation) (tl : event) (ind : nat),
	rem_op_aux (comp ++ tl :: nil) ind = rem_op
Proof.
	intros. unfold rem_op_aux. rewrite (fold_right_app keep comp (tl :: nil) (ind, nil)).
	simpl. unfold keep. admit.*)

Lemma n_rem_op:
	forall (comp : computation) (ind : nat),
	fst (rem_op_aux comp ind) = length comp.
Proof.
	Admitted.

Lemma rem_op_tl:
	forall (comp : computation) (tl : event) (a : nat),
	a = length (comp ++ tl :: nil) - 1 ->
	rem_op (comp ++ tl :: nil) a = comp.
Proof.
	intros comp. remember (length comp) as n. generalize dependent comp.
	induction n.
	- admit.
	- intros. unfold rem_op. unfold rem_op_aux.
		rewrite (fold_left_app (keep a) comp (tl :: nil) (0, nil)).
		simpl. remember (fold_left (keep a) comp (0, nil)) as tmp.
		unfold keep. 

		assert
	
	rewrite H. assert (length (comp ++ tl :: nil) - (length (comp ++ tl :: nil) - 1) - 1 = 0). { lia. }
	rewrite H0. rewrite (fold_right_app keep comp (tl :: nil) (0, nil)).
	assert (fold_right keep (0, nil) (tl :: nil) = (0, nil)). { reflexivity. }
	rewrite H1. unfold rem_op in IHn. unfold rem_op_aux in IHn.
	
	
	
	intros. unfold rem_op. unfold rem_op_aux. 
	rewrite (fold_right_app keep comp (tl :: nil) (a, nil)).

	unfold rem_op in IHn. unfold rem_op_aux in IHn. 

	 rewrite (rem_step comp tl a). unfold keep.
		unfold rem_op in IHn. 

Lemma rem_op_comp:
	forall (comp : computation) (tl : event) (state : run_op_state) (a : nat),
	a < length (comp ++ tl :: nil) ->
	rem_op (comp ++ tl :: nil) a = (rem_op comp a) ++ tl :: nil.
Proof.
	intros comp. remember (length comp) as n. generalize dependent comp.
	induction n.
...
Lemma affects_state:
	...
	b < length comp ->
	after comp a b -> 
	let op_states := snd (fst (apply_operations comp (0, state, fun _ => 0))) in
	let op_states_rem := snd (fst (apply_operations (rem_op comp a) (0, state, fun _ => 0))) in
	op_states b <> op_states_rem b.
Proof.
	intros comp. remember (length comp) as n. generalize dependent comp.
	destruct n.
	...
	- destruct n.
		...
		{
			intros comp Hlen a b state Hlen_bound Ha_bound Hb_bound Hafter.
			destruct (list_tl comp) as [comp' Hcomp']. {
				symmetry in Hlen. rewrite Hlen. lia.
			}
			{
				destruct Hcomp' as [tl Hcomp]. {
					pose proof (comp'_len comp comp' tl  Hcomp) as Hcomp'.
					rewrite Hcomp. Check op_step. rewrite (op_step comp' tl (0, state, fun _ => 0)). simpl.
		}
...
Lemma affects1:
	...
	b < length comp ->
	after comp a b -> 
	let op_states := apply_operations comp state in
	let op_states_rem := apply_operations (rem_op comp a) state in
	op_states b <> op_states_rem b.
Proof.
	intros comp. remember (length comp) as n. generalize dependent comp.
	destruct n.
	...
	- destruct n.
		...
		{
			intros comp Hlen a b state Hlen_bound Ha_bound Hb_bound Hafter. simpl.
			unfold apply_operations.
		}
...
Definition add_operation (cur_op : operation_index * user) (op_timestamps : time_stamps) (op : database_operation) (acc : operation_index list * operation_index) : (operation_index list * operation_index) :=
	let cur_op_ind := fst cur_op in
	let user := snd cur_op in
	let clean_inds := fst acc in
	let op_ind := snd acc in
	match op_timestamps op_ind user <? op_timestamps cur_op_ind user with
	| true => (op_ind :: clean_inds, op_ind + 1)
	| false => (clean_inds, op_ind + 1)
	end.

Definition cleanup_computation (comp: database_computation) (cur_op_ind : operation_index) : database_computation :=
	let user := event_process comp cur_op_ind in
	let op_timestamps : time_stamps := 
		snd (run_computation comp init_state) in
	fold_right (add_operation (cur_op_ind, user) op_timestamps) comp (nil, 0).

Theorem correct_cleanup:
	forall (comp: database_computation) (op_ind : operation_index) (i : operation_index),
	let clean_inds := fst (cleanup_computation comp op_ind) in
	after comp i op_ind <-> In i clean_comp.

(* For a sequence of sequential events, would be nice *)

(*
	When rolling back database operations, it is important whether or not a -> b 
	or they occured sequentially
*)
Theorem preserve_causality:
	forall (i j : operation_index) (op1 op2 : database_operation) (u : user) (comp : database_computation),
	op1 = nth i comp (Event 0) ->
	op2 = nth j comp (Event 0) ->
	u = event_process database_operation i ->
	let event_timestamps : time_stamps := 
		snd (run_computation comp init_state) in
	event_timestamps i u < event_timestamps j u ->
	after comp i j.
	(* add_operation will never cut out an event that caused current state (cur_op) *)
Proof.
	Admitted.

Definition add_operation (cur_op : operation_index * user) (op_timestamps : time_stamps) (op : database_operation) (acc : database_computation * operation_index) : (database_computation * operation_index) :=
	let cur_op_ind := fst cur_op in
	let user := snd cur_op in
	let clean_comp := fst acc in
	let op_ind := snd acc in
	match op_timestamps op_ind user <? op_timestamps cur_op_ind user with
	| true => (op :: clean_comp, op_ind + 1)
	| false => (clean_comp, op_ind + 1)
	end.
